...
#define HADOOP_DFS      "org/apache/hadoop/hdfs/DistributedFileSystem"
#define DATANODE_INFO   "org/apache/hadoop/hdfs/protocol/DatanodeInfo"
#define DATANODE_REPORT "org/apache/hadoop/hdfs/protocol/HdfsConstants$DatanodeReportType"
...
static int getDataNodeInfo(JNIEnv *env, jobject jDataNodeInfo, hdfsDataNodeInfo *dataNodeInfos)
{
    jvalue jVal;
    ...
    if (jExc)  { 
        errno = printExceptionAndFree(env, jExc, PRINT_EXC_ALL,
        "org.apache.hadoop.fs.DataNodeInfo::getHostname");
        return -1;
    }
    if (jVal.l == NULL) 
    ...
    if (jExc)   {
        errno = printExceptionAndFree( env, jExc,PRINT_EXC_ALL,
        "org.apache.hadoop.fs.DataNodeInfo::getCapacity");
        return -1;
    }

    dataNodeInfos->capacity = jVal.j;
    jExc = invokeMethod(env, &jVal, INSTANCE, jDataNodeInfo, DATANODE_INFO,
        "getDfsUsed", "()J");
    
    if (jExc) {
        errno = printExceptionAndFree( env,jExc,PRINT_EXC_ALL,
        "org.apache.hadoop.fs.DataNodeInfo::getDfsUsed");
        return -1;
    }

    dataNodeInfos->dfsUsed = jVal.j;
    jExc = invokeMethod(env, &jVal, INSTANCE, jDataNodeInfo, DATANODE_INFO,
        "getLastUpdate","()J");

    if (jExc) {
        errno = printExceptionAndFree(env, jExc,PRINT_EXC_ALL,
        "org.apache.hadoop.fs.DataNodeInfo::getLastUpdate");
        return -1;
    }
    dataNodeInfos->lastUpdate = jVal.i;
    jExc = invokeMethod(env, &jVal, INSTANCE, jDataNodeInfo, DATANODE_INFO,
        "getNetworkLocation", "()Ljava/lang/String;");

    if (jExc) {
         errno = printExceptionAndFree( env,jExc,PRINT_EXC_ALL,
         "org.apache.hadoop.fs.DataNodeInfo::getNetworkLocation");
         return -1;
    }
    if (jVal.l == NULL)
         strcpy(dataNodeInfos->location,"");
    else
        jExc =  newCStr(env, jVal.l, &dataNodeInfos->location);
    ...
    if  (jExc) {
         errno = printExceptionAndFree( env, jExc,PRINT_EXC_ALL,
         "org.apache.hadoop.fs.DataNodeInfo::getRemaining");
         return -1;
     }

     dataNodeInfos->remaining = jVal.j;
     jExc = invokeMethod(env, &jVal, INSTANCE, jDataNodeInfo, DATANODE_INFO,
         "getXceiverCount", "()I");

     if  (jExc) {
         errno =printExceptionAndFree( env,jExc,PRINT_EXC_ALL,
         "org.apache.hadoop.fs.DataNodeInfo::getXceiverCount");
         return -1;
     }
     dataNodeInfos->xceiverCount = jVal.i;
     return 0;
}

static char * enum_to_string(enum dataNodeReportType type ){
    switch(type){
         case DN_REPORT_ALL:
            return "ALL";
         case DN_REPORT_LIVE:
...
hdfsDataNodeInfo **hdfsGetDataNodeInfo(hdfsFS fs, enum dataNodeReportType dataNodeType,int* numEntries)

{
    ...
    *numEntries=0;
    jfieldID fid;
    jobject enumType = NULL;
    jobjectArray jDatanodeInfos = NULL;
    hdfsDataNodeInfo ** dataNodeInfoList = 0;
    jsize i;
    jobject tmpDataNodeInfo;
    jobject jFS = (jobject)fs;
    ...
    jclass cls=(*env)->FindClass(env, DATANODE_REPORT);

    fid = (*env)->GetStaticFieldID(env, cls,enum_to_string(dataNodeType), "Lorg/apache/hadoop/hdfs/protocol/HdfsConstants$DatanodeReportType;");
    if (fid == NULL) {
         printf("Field not found\n");
         goto done;
    }
    enumType  = (*env)->GetStaticObjectField(env, cls, fid);
    if(enumType == NULL) {
         printf("Object Field not found\n");
         goto done;
    }
    jExc = invokeMethod(env, &jVal,  INSTANCE, jFS,
                     HADOOP_DFS, "getDataNodeStats",
                     JMETHOD1(JPARAM(DATANODE_REPORT), JARRPARAM(DATANODE_INFO)),enumType);
    if (jExc) {
    ...
    jDatanodeInfos = jVal.l;
    //Figure out no of entries in jDataNodeInfos
    jsize jNumDataNodeInfos = (*env)->GetArrayLength(env, jDatanodeInfos);
    ...
    //Save DataNodeInfo in dataNodeInfoList
    for (i=0; i < jNumDataNodeInfos; i++) {
        tmpDataNodeInfo = (*env)->GetObjectArrayElement(env,jDatanodeInfos, i);
        dataNodeInfoList[i] = (hdfsDataNodeInfo*) malloc(sizeof(hdfsDataNodeInfo));
        if (!dataNodeInfoList[i]) {
            fprintf(stderr, "dataNodeInfoList: out of memory");
            errno = ENOMEM;
            goto done;
        }
        if (getDataNodeInfo(env, tmpDataNodeInfo, dataNodeInfoList[i])) {
            hdfsFreeDataNodeInfo(dataNodeInfoList, jNumDataNodeInfos);
            destroyLocalReference(env, tmpDataNodeInfo);
            dataNodeInfoList = NULL;
            goto done;
        }
        destroyLocalReference(env, tmpDataNodeInfo);
    }
    *numEntries=jNumDataNodeInfos;

    done:
    //Delete unnecessary local references
    destroyLocalReference(env, jDatanodeInfos);
    destroyLocalReference(env, enumType);

    return dataNodeInfoList;
}

void hdfsFreeDataNodeInfo(hdfsDataNodeInfo **hdfsDataNodeInfos, int numEntries)
{
    //Free the hostname and location
    int i;
    for (i=0; i < numEntries; ++i) {
        ...
        free(hdfsDataNodeInfos[i]);
    }
    //Free entire block
    free(hdfsDataNodeInfos);
}
